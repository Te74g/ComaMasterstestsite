...
const STORAGE_KEY = "comamasters_ranking_portal_v1";

const DEFAULT_STATE = {
  site: {
    title: "ComaMasters Ranking Portal",
    subtitle: "Tonamelで確定した大会結果から累計ランキングを自動生成",
    tonamelHubUrl: "https://x.com/ComaMastersinfo"
  },
  tournaments: [],
  updatedAt: null
};
...
function onSaveSiteSettings(event) {
  ...
  if (!title || !subtitle || !tonamelHubUrl) {
    window.alert("サイト設定をすべて入力してください。");
    return;
  }
...
function onAddTournament(event) {
  ...
  if (!name || !date || !venue || !sourceUrl || !resultsText) {
    window.alert("大会情報と結果リストをすべて入力してください。");
    return;
  }

  const parseResult = parseResultsText(resultsText);
  if (!parseResult.ok) {
    window.alert(parseResult.message);
    return;
  }
...
function onAdminTableClick(event) {
  ...
  const confirmed = window.confirm(`「${tournament.name}」を削除しますか？`);
  if (!confirmed) {
    return;
  }
...
function onReset() {
  const confirmed = window.confirm("全データを削除して初期状態に戻します。よろしいですか？");
  if (!confirmed) {
    return;
  }
...
function renderHeader() {
  ...
  if (state.updatedAt) {
    el.lastUpdated.textContent = `Last update: ${formatDateTime(state.updatedAt)}`;
  } else {
      ...
      .map((row) => {
        ...
        return `
          <tr>
            ...
            <td>${formatNumber(row.latestPts)}</td>
          </tr>
        `;
      })
      .join("");
...
function renderTournamentCards() {
  ...
  el.tournamentCards.innerHTML = sorted
    .map((tournament) => {
      ...
      const topList = top3
        .map((entry) => `<li>${entry.rank}位 ${escapeHtml(entry.player)} (${formatNumber(entry.points)} pts)</li>`)
        .join("");

      return `
        <article class="tournament-card">
          <div class="tournament-card__head">
            <h3 class="tournament-card__name">${escapeHtml(tournament.name)}</h3>
            <a class="action-link" href="${escapeAttr(tournament.sourceUrl)}" target="_blank" rel="noopener noreferrer">Tonamel</a>
          </div>
          <p class="tournament-card__meta">${formatDate(tournament.date)} / ${escapeHtml(tournament.venue)} / ${tournament.results.length} entries</p>
    ...
    .map((tournament) => {
      return `
        <tr>
          ...
          <td>${escapeHtml(tournament.venue)}</td>
          <td>${tournament.results.length}</td>
          <td>
            <button class="btn btn--danger" type="button" data-action="delete-tournament" data-id="${escapeAttr(tournament.id)}">
              Delete
            </button>
          </td>
        </tr>
